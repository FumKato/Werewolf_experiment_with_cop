...
suite('Server: RoleSetA', function() {
	test('createRoles: quota is 4', function(done, server) {
		server.eval(function() {
			...
			var results = new RoleSetA().createRoles(villageSettings);
	...
	test('createRoles: quota is 8', function(done, server) {
		...
		server.on('checkRoleSet', function(results, expects){
			var flag = results.villager > 0 && results.villager <= expects.villager;
			assert.equal(flag, true);
			flag = results.wolf >= 1 && results.wolf <= expects.wolf;
			assert.equal(flag, true);
			assert.equal(results.seer, expects.seer);
			assert.equal(flag, true);
			assert.equal(results.medium, expects.medium);
			...
			assert.equal(results.cat, expects.cat);
			assert.equal(results.girl, expects.girl);
			assert.equal(results.girl, expects.wizard);
			assert.equal(results.sum, expects.sum);
		});
	...
	test('createRoles: quota is 10 with wizard: wizard appears', function(done, server) {
		server.eval(function() {
			var villageSettings = {
				quota: 10,
				cat: false,
	...
	test('createRoles: quota is 14 with cat and girl and wizard: cat and girl and wizard appears', function(done, server) {
		server.eval(function() {
			var villageSettings = {
				quota: 14,
				cat: true,
	...
	test('createRoleBreakdown: quota is 4', function(done, server) {
		server.eval(function() {
			...
			var results = new RoleSetA().createRoleBreakdown(villageSettings);
	...
	test('createRoleBreakdown: quota is 8', function(done, server) {
		server.eval(function() {
			var villageSettings = {
				quota: 8,
				cat: false,
